...
def train_one_epoch(model,dataloader,loss_fn,optimizer,device):
    ...
    model.to(device)
    model.train()
    for eye_image,mouth_image,label in tqdm(dataloader,desc="Training",mininterval=1):
       
        eye_image = eye_image.to(device)
        mouth_image = mouth_image.to(device)
        label = label.to(device)
        pred = model(eye_image,mouth_image)
...
def validate_one_epoch(model,dataloader,loss_fn,device):
    ...
    model.eval()
    with torch.no_grad():
        for eye_image,mouth_image,label in tqdm(dataloader,desc="Validation",mininterval=1):
            eye_image = eye_image.to(device)
            mouth_image = mouth_image.to(device)
            label = label.to(device)
            pred = model(eye_image,mouth_image)
...
def dual_stream_train(model,train_dataloader,val_dataloader,loss_fn,optimizer,num_epochs,device,scheduler,save_name):
